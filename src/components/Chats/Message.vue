        ...
        <div class="text-main">
            <div class="text-group" :class="{'me': message.uid == user.uid}">
                <div class="text text-white" @click.prevent.stop="handleClick" @contextmenu.prevent.stop="handleClick">
                    <p v-if="message.type == 'text'" :class="{'read': message.isRead, 'notRead': !message.isRead}" ref="textMessage">{{message.message}}</p>
            ...
            <span>
                ...
                <div class="inline" v-if="message.uid == user.uid">
                    <i v-if="message.isRead" class="bx bx-check-double text-blue-500" :class="{'order-1': message.uid != user.uid}"></i>
...
export default {
    ...
    setup({message, chatId}) {
        ...
        onMounted(async () => {
            console.log(message.id, message.uid != user.uid, chatId)
